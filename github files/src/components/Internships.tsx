...
export const Internships: React.FC = () => {
  ...
  const displayedInternships = showAll 
    ? visibleInternships 
    : visibleInternships.slice(0, INITIAL_SHOW_COUNT);
  ...
  return (
    <section id="internships" className="relative bg-transparent py-14 sm:py-16 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {displayedInternships.map((internship, index) => (
            <motion.div 
              ...
              tabIndex={0}
              className="group relative bg-black/60 p-5 sm:p-6 md:p-8 rounded-xl border border-gray-800 hover:border-cyan-400 hover:shadow-[0_0_30px_rgba(0,240,255,0.7),_0_0_50px_rgba(0,240,255,0.35)] hover:-translate-y-1 active:border-cyan-400 active:shadow-[0_0_35px_rgba(0,240,255,0.8),_0_0_60px_rgba(0,240,255,0.4)] focus:border-cyan-400 focus:shadow-[0_0_35px_rgba(0,240,255,0.8),_0_0_60px_rgba(0,240,255,0.4)] transition-all duration-300 flex flex-col justify-between min-h-[auto] sm:min-h-[350px] cursor-pointer outline-none"
            >
              {/* Top Section */}
              <div>
              ...
              <div className="mt-auto">
                ...
                <button 
                  onClick={(e) => {
                    e.preventDefault();
                    const certUrl = internship.certificateUrl || (internship as any).certificateFile || (internship as any).url || (internship.role ? `/internship_cert_${internship.id || '1'}.png` : null);
                    if (certUrl) {
                      setViewerCert({ title: `${internship.role} (${internship.company})`, imageUrl: certUrl });
                    } else {
        ...
        {/* View More / View All Internships Button */}
        {visibleInternships.length > INITIAL_SHOW_COUNT && (
          <div className="mt-12 flex justify-center">
            <button
              onClick={() => setShowAll(!showAll)}
              className="px-8 py-4 bg-cyan-500/10 border border-cyan-400/60 hover:border-cyan-400 text-cyan-400 hover:text-white hover:bg-cyan-500/20 font-mono text-xs font-bold tracking-widest transition-all duration-300 cursor-pointer uppercase shadow-[0_0_20px_rgba(0,240,255,0.3)] hover:scale-[1.02]"
              style={{
                clipPath: 'polygon(12px 0, calc(100% - 12px) 0, 100% 12px, 100% calc(100% - 12px), calc(100% - 12px) 100%, 12px 100%, 0 calc(100% - 12px), 0 12px)'
              }}
            >
              {showAll ? 'SHOW LESS INTERNSHIPS' : `VIEW MORE INTERNSHIPS (${visibleInternships.length - INITIAL_SHOW_COUNT} MORE)`}
            </button>
          </div>
        )}
      </div>
