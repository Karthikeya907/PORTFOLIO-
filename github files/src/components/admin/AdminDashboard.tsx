...
import { 
  ...
  X, Check, AlertTriangle, Upload, Download, ExternalLink, Database
} from 'lucide-react';
import { usePortfolio } from '../../context/PortfolioContext';
import type { MissionData } from '../ProjectModal';
import type { InternshipItem, CertItem, SkillCategory, SocialLink, SiteSettings, ResumeInfo } from '../../context/PortfolioContext';
...
type TabType = 'profile' | 'projects' | 'skills' | 'certifications' | 'internships' | 'resume' | 'contact' | 'social' | 'about' | 'settings' | 'files' | 'storage';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
  ...
  // Local draft states for each section with safe default fallbacks
  const defaultProfile = { name: '', title: '', subtitle: '', location: '', status: '', bio: '', email: '', phone: '', whatsapp: '', shortIntro: '', github: '', linkedin: '', avatarUrl: '' };
  const defaultAbout = { shortIntro: '', description: '', currentFocus: '' };
  const defaultSettings: SiteSettings = { siteTitle: '', heroHeading: '', heroSubtitle: '', adminPassword: '', showSections: {}, enableAnimations: true, enableSound: true };
  const defaultResume: ResumeInfo = { available: true, fileUrl: '/resume.pdf', filename: 'Naga_Karthikeya_Resume.pdf', lastUpdated: '' };
  ...
  const [draftResume, setDraftResume] = useState<ResumeInfo>(resume || defaultResume);
  const [draftSocial, setDraftSocial] = useState<SocialLink[]>(socialLinks || []);
  const [draftAbout, setDraftAbout] = useState(about || defaultAbout);
  const [draftSettings, setDraftSettings] = useState<SiteSettings>(settings || defaultSettings);
  ...
  const handleChangePassword = async () => {
    ...
    const updatedSettings = { ...(draftSettings || defaultSettings), adminPassword: newPassInput.trim() };
    const ok = await updateSection('settings', updatedSettings);
    if (ok) {
      setDraftSettings(updatedSettings);
      showToast('Password updated! Old password deactivated.', 'success');
      setNewPassInput('');
    } else {
  ...
  useEffect(() => {
    ...
    setDraftResume(resume || defaultResume);
    setDraftSocial(socialLinks || []);
    setDraftAbout(about || defaultAbout);
    setDraftSettings(settings || defaultSettings);
    setHasUnsaved(false);
  ...
  const storageStats = React.useMemo(() => {
    try {
      const certsList = Array.isArray(draftCerts) ? draftCerts : [];
      const internshipsList = Array.isArray(draftInternships) ? draftInternships : [];
      const projectsList = Array.isArray(draftProjects) ? draftProjects : [];

      let resumeBytes = (draftResume && typeof draftResume.fileUrl === 'string') ? draftResume.fileUrl.length : 0;
      
      let certBytes = 0;
      certsList.forEach((c: any) => { if (c && typeof (c.imageUrl || c.certificateFile || c.url) === 'string') certBytes += (c.imageUrl || c.certificateFile || c.url).length; });

      let internshipBytes = 0;
      internshipsList.forEach(i => { if (i && typeof i.certificateUrl === 'string') internshipBytes += i.certificateUrl.length; });

      let projectBytes = 0;
      projectsList.forEach((p: any) => {
        if (p) {
          if (typeof p.image === 'string') projectBytes += p.image.length;
          if (Array.isArray(p.gallery)) p.gallery.forEach((g: any) => { if (typeof g === 'string') projectBytes += g.length; });
        }
      });

      const totalBytes = resumeBytes + certBytes + internshipBytes + projectBytes;
      const usedMB = totalBytes / (1024 * 1024);
      const totalCapacityMB = 1000.0; // 1,000 MB Free Tier Bucket Limit
      const remainingMB = Math.max(0, totalCapacityMB - usedMB);
      const percentage = Math.min(100, (usedMB / totalCapacityMB) * 100);

      return {
        usedMB: usedMB.toFixed(2),
        totalMB: totalCapacityMB.toFixed(2),
        remainingMB: remainingMB.toFixed(2),
        percentage: percentage < 0.01 && usedMB > 0 ? '0.01' : percentage.toFixed(2),
        resumeMB: (resumeBytes / (1024 * 1024)).toFixed(2),
        certMB: (certBytes / (1024 * 1024)).toFixed(2),
        internshipMB: (internshipBytes / (1024 * 1024)).toFixed(2),
        projectMB: (projectBytes / (1024 * 1024)).toFixed(2),
        totalBytes
      };
    } catch (e) {
      return {
  ...
  }, [draftResume, draftCerts, draftInternships, draftProjects]);
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <aside className="w-64 border-r border-gray-800 bg-gray-950/60 p-4 flex flex-col gap-1 overflow-y-auto">
            {[
              ...
              { id: 'storage', label: '12. Storage Usage & Meter', icon: Database },
            ].map((item) => {
            ...
            {/* Live Storage Usage Meter Sidebar Widget */}
            <div 
              onClick={() => handleTabChange('storage')}
              className="mt-auto pt-4 border-t border-gray-800/80 p-3 bg-black/60 rounded-lg border border-cyan-500/20 hover:border-cyan-400 transition-all cursor-pointer group"
            >
              <div className="flex justify-between items-center text-[10px] font-mono text-gray-400 mb-1.5">
                <span className="flex items-center gap-1 text-cyan-400 font-bold uppercase">
                  <Database size={12} /> SUPABASE STORAGE
                </span>
                <span className="font-bold text-white">{storageStats.percentage}%</span>
              </div>
              <div className="w-full bg-gray-900 h-2 rounded-full overflow-hidden border border-gray-800">
                <div 
                  className="bg-cyan-400 h-full transition-all duration-500 shadow-[0_0_10px_rgba(0,240,255,0.6)]"
                  style={{ width: `${Math.max(2, parseFloat(storageStats.percentage))}%` }}
                />
              </div>
              <div className="flex justify-between items-center text-[10px] font-mono text-gray-500 mt-2">
                ...
                <span className="text-cyan-400/90 font-bold">{storageStats.remainingMB} MB Free</span>
              </div>
            </div>
          </aside>
          ...
          <main className="flex-1 p-6 md:p-10 overflow-y-auto bg-black/90 custom-scrollbar">
              ...
              <div className="max-w-3xl space-y-6">
                ...
                <div className="bg-gray-950/80 p-6 border border-gray-800 space-y-6">
                  ...
                  <div className="border-t border-gray-800 pt-6">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative flex-1">
                        <input
                          ...
                          className="w-full px-4 py-2.5 pr-10 bg-black border border-gray-800 text-white font-mono text-sm outline-none focus:border-cyan-400"
                        />
                        <button
                          type="button"
                    ...
                    <span className="text-[11px] font-mono text-emerald-400/90 mt-2 block">
                      🔒 Security Note: When updated, the old password is immediately invalidated and only your new password will be active.
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* 12. STORAGE USAGE & METRICS */}
            {activeTab === 'storage' && (
              <div className="max-w-4xl space-y-8">
                <div className="border-b border-gray-800 pb-4">
                  <h2 className="text-xl font-bold text-white tracking-wider flex items-center gap-3 uppercase">
                    <Database className="text-cyan-400" size={24} /> SUPABASE STORAGE & CAPACITY METRICS
                  </h2>
                  <p className="text-xs font-mono text-gray-400">Live storage monitoring for Supabase Free Tier (1,000 MB Bucket Storage & 500 MB DB Limit).</p>
                </div>

                {/* Metric Summary Cards Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="p-5 bg-gray-950/80 border border-cyan-500/30 rounded-xl space-y-2">
                    <span className="text-[10px] font-mono text-cyan-400 font-bold uppercase tracking-wider block">USED STORAGE</span>
                    <div className="text-2xl font-bold text-white font-mono">{storageStats.usedMB} <span className="text-sm font-normal text-gray-400">MB</span></div>
                    <span className="text-[10px] font-mono text-gray-500 block">Uploaded Assets Size</span>
                  </div>

                  <div className="p-5 bg-gray-950/80 border border-emerald-500/30 rounded-xl space-y-2">
                    <span className="text-[10px] font-mono text-emerald-400 font-bold uppercase tracking-wider block">FREE CAPACITY</span>
                    <div className="text-2xl font-bold text-white font-mono">1,000 <span className="text-sm font-normal text-gray-400">MB</span></div>
                    <span className="text-[10px] font-mono text-gray-500 block">Supabase Free Storage Limit</span>
                  </div>

                  <div className="p-5 bg-gray-950/80 border border-cyan-500/30 rounded-xl space-y-2">
                    <span className="text-[10px] font-mono text-cyan-400 font-bold uppercase tracking-wider block">REMAINING FREE SPACE</span>
                    <div className="text-2xl font-bold text-cyan-300 font-mono">{storageStats.remainingMB} <span className="text-sm font-normal text-gray-400">MB</span></div>
                    <span className="text-[10px] font-mono text-gray-500 block">Available for future uploads</span>
                  </div>

                  <div className="p-5 bg-gray-950/80 border border-purple-500/30 rounded-xl space-y-2">
                    <span className="text-[10px] font-mono text-purple-400 font-bold uppercase tracking-wider block">DATABASE STATUS</span>
                    <div className="text-lg font-bold text-emerald-400 font-mono flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
                      <span>ONLINE</span>
                    </div>
                    <span className="text-[10px] font-mono text-gray-500 block">500 MB PostgreSQL Active</span>
                  </div>
                </div>

                {/* Storage Capacity Gauge Progress Meter */}
                <div className="p-6 bg-gray-950/80 border border-cyan-500/30 rounded-xl space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider">SUPABASE BUCKET CAPACITY GAUGE</h3>
                      <p className="text-xs font-mono text-gray-400">Real-time percentage consumption of your 1,000 MB free allocation.</p>
                    </div>
                    <span className="text-xl font-bold text-cyan-400 font-mono">{storageStats.percentage}%</span>
                  </div>

                  <div className="w-full bg-black h-4 rounded-full overflow-hidden border border-gray-800 p-0.5">
                    <div 
                      className="bg-gradient-to-r from-cyan-500 to-emerald-400 h-full rounded-full transition-all duration-700 shadow-[0_0_15px_rgba(0,240,255,0.6)]"
                      style={{ width: `${Math.max(1, parseFloat(storageStats.percentage))}%` }}
                    />
                  </div>
                  ...
                  <div className="flex justify-between text-xs font-mono text-gray-400 pt-1">
                    <span>0 MB (0%)</span>
                    <span>500 MB (50%)</span>
                    <span>1,000 MB (100% Free Limit)</span>
                  </div>
                </div>

                {/* Storage Item Breakdown Table */}
                <div className="p-6 bg-gray-950/80 border border-gray-800 rounded-xl space-y-4">
                  <h3 className="text-sm font-bold text-white font-mono uppercase tracking-wider">ASSET STORAGE BREAKDOWN</h3>
                  <div className="divide-y divide-gray-800/60 font-mono text-xs">
                    <div className="py-3 flex justify-between items-center">
                      <span className="text-gray-300">Resume PDF Document</span>
                      <span className="text-cyan-400 font-bold">{storageStats.resumeMB} MB</span>
                    </div>
                    <div className="py-3 flex justify-between items-center">
                      <span className="text-gray-300">Certifications Images ({draftCerts.length} items)</span>
                      <span className="text-cyan-400 font-bold">{storageStats.certMB} MB</span>
                    </div>
                    <div className="py-3 flex justify-between items-center">
                      <span className="text-gray-300">Internship Certificates ({draftInternships.length} items)</span>
                      <span className="text-cyan-400 font-bold">{storageStats.internshipMB} MB</span>
                    </div>
                    <div className="py-3 flex justify-between items-center">
                      <span className="text-gray-300">Project Gallery Media ({draftProjects.length} projects)</span>
                      <span className="text-cyan-400 font-bold">{storageStats.projectMB} MB</span>
                    </div>
                  </div>
                </div>
              </div>
