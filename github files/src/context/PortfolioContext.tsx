...
import type { MissionData } from '../components/ProjectModal';
import {
  fetchSectionData,
  saveSectionData,
...
export const PortfolioProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      if (isSupabaseConfigured) {
        const [
          ...
          fetchSectionData('profile'),
          fetchSectionData('socialLinks'),
          fetchSectionData('about'),
          fetchSectionData('resume'),
          fetchSectionData('settings'),
        ]);

        if (dbProjects) setProjects(normalizeProjects(dbProjects));
        else saveSectionData('projects', projectsData);

        if (dbInternships) setInternships(dbInternships);
        else saveSectionData('internships', internshipsData);

        if (dbCertifications) setCertifications(dbCertifications);
        else saveSectionData('certifications', certificationsData);

        if (dbSkills) setSkills(dbSkills);
        else saveSectionData('skills', skillsData);

        if (dbProfile) setProfile(dbProfile);
        else saveSectionData('profile', profileData.profile);

        if (dbSocialLinks) setSocialLinks(dbSocialLinks);
        else saveSectionData('socialLinks', profileData.socialLinks);

        if (dbAbout) setAbout(dbAbout);
        else saveSectionData('about', profileData.about);

        if (dbResume) setResume(dbResume);
        else saveSectionData('resume', profileData.resume);

        if (dbSettings) setSettings(dbSettings);
        else saveSectionData('settings', profileData.settings);
      }

      // Try local/custom API endpoint as fallback
  ...
  const updateSection = async (section: string, data: any): Promise<boolean> => {
    ...
    if (isSupabaseConfigured) {
      supabaseSaved = await saveSectionData(section, data);
    }
    ...
    try {
      ...
      const json = await res.json();
      return json.success === true || supabaseSaved;
    } catch (err) {
      console.warn('Backend API update failed, returning Supabase save status:', err);
      return supabaseSaved || true;
    }
  };

  const uploadFile = async (fileName: string, fileData: string, fileCategory = 'media') => {
    if (isSupabaseConfigured) {
      const result = await uploadFileToSupabase(fileData, fileName, fileCategory);
    ...
    try {
      ...
      console.error('Failed to upload file to backend server:', err);
    }
